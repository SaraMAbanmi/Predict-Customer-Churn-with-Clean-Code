...
def perform_eda(data_frame, show_flag:bool=True):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join("./images/eda", 'CustomerAge.png'))
    if show_flag:
        plt.show(block=False)

    plt.figure(figsize=(20, 10))
    data_frame.Marital_Status.value_counts('normalize').plot(kind='bar')
    plt.savefig(os.path.join("./images/eda", 'MaritalStatus.png'))

    plt.figure(figsize=(20, 10))
    sns.histplot(data_frame['Total_Trans_Ct'], stat='density', kde=True)
    plt.savefig(os.path.join("./images/eda", 'TotalTransHist.png'))

    plt.figure(figsize=(20, 10))
    sns.heatmap(data_frame.corr(), annot=False, cmap='Dark2_r', linewidths=2)
    plt.savefig(os.path.join("./images/eda", 'CorrHeatmap.png'))
    if show_flag:
        plt.show()
